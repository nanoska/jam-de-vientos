...
export default function AdminDashboard() {
  ...
  const filteredThemes = themes.filter((theme) => {
    ...
    return (
      ...
      theme.artist.toLowerCase().includes(searchLower) ||
      theme.key.toLowerCase().includes(searchLower)
    )
  })
                ...
                <div className="space-y-4">
                  {filteredThemes.length === 0 ? (
                    <div className="text-center py-8 text-gray-500">
                      {searchTerm ? `No se encontraron temas que coincidan con "${searchTerm}"` : "No hay temas disponibles"}
                    </div>
                  ) : (
                      ...
                      >
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-sm text-gray-600 mb-3 truncate">{theme.artist}</p>
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                              <div className="flex items-center space-x-1">
                                ...
                                >
                                  {theme.image !== "/placeholder.svg" ? "Img" : "Sin Img"}
                                </Badge>
                              </div>
                              <div className="flex items-center space-x-1">
                              ...
                              <div className="flex items-center space-x-1">
                                ...
                                >
                                  {theme.files.some((f) => f.type === "pdf")
                                    ? `${theme.files.filter((f) => f.type === "pdf").length} PDF`
                                    : "Sin PDF"}
                                </Badge>
                              </div>
                              <div className="text-xs text-gray-500 truncate sm:text-right">ID: {theme.id}</div>
                            </div>
                          </div>
